...
- `plainText(AtlasCase, CitationContext)` — three lines, one per citable entity: `Image:` the atlas CFF citation (author, title, DOI, viewer URL, accessed date/catalog SHA), `Software:` the extension CFF citation (DOI + version), `Platform:` the QuPath/Bankhead 2017 citation (DOI). Organ/category are not inlined here — they're carried in the manifest CSV/MD and the RIS `N1` field instead.
...
- `methodsParagraph(List<AtlasCase>, CitationContext)` — one paste-ready sentence, correctly pluralized: "N whole-slide image(s) (M case(s)) from the Patoloji Atlası (Balcı S, DOI) were reviewed in QuPath (Bankhead et al. 2017, DOI) via the qupath-extension-atlas v<version> (DOI), accessed <date> (catalogue snapshot <sha>). Exact slide URLs are listed in the accompanying manifest." No per-stain/organ breakdown is inlined — that detail is carried in the accompanying manifest CSV/MD, not the methods paragraph itself.
